<template>
  <div class="relative flex justify-center slide mt-7">
    <div
      class="carousel-indicators absolute bottom-0 flex h-24 w-4/5 justify-center items-center"
    >
      <ol class="z-50 flex w-4/12 justify-center">
        <li
          ...
          :class="`${active === index ? 'active' : 'bg-white'}`"
        ></li>
      </ol>
    </div>
    <div
      class="carousel-inner relative rounded-xl overflow-hidden w-11/12 max-h-[700px]"
    >
      <div
        v-for="(item, index) in popularList"
        :key="index"
...
export default {
  setup() {
    ...
    return { store, items, active, popularList, API_BASE_URL };
  },
  async created() {
    let index = 0;
    setInterval(() => {
