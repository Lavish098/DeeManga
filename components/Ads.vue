...
<div class="relative flex justify-center slide mt-7">
    <div class="carousel-indicators absolute bottom-0 flex h-24 w-4/5 justify-center items-center">
      <ol class="z-50 flex w-4/12 justify-center">
        <li v-for="(item, index) in mangaRecommendList" :key="index" class="md:w-4 md:h-4 bg-gray-300 rounded-full cursor-pointer mx-2"
        :class="`${active === index ? 'active' : 'bg-white'}`"></li>
      </ol>
    </div>
    <div class="carousel-inner relative rounded-xl overflow-hidden w-11/12 max-h-[700px]">
      <div v-for="(item, index) in mangaRecommendList" :key="index" :id="`slide-${index}`" :class="`${active === index ? 'active' : 'left-full'}`" 
      class="carousel-item inset-0 relative w-full transform transition-all duration-500 ease-in-out">
        <img class="block w-auto h-1/2 z-10" :src="item.img" alt="First slide" />
...
const mangaRecommendList = computed(() => {
  ...
  return{ store, items, active, mangaRecommendList}
},
 async created(){
    let index = 0;
    setInterval(() => {
      if (index > this.mangaRecommendList.length - 1) {
        index = 0;
      }
      this.active = index;
      index++;
    }, 2000);
    console.log(this.store.mangaRecommendList);
  },
