  ...
  <div class="flex flex-wrap justify-center items-center m-5">
    <div class="flex justify-center items-center p-[10px]">
      <button
        ...
        class="bg-slate-600 py-[10px] px-[20px] rounded-md mr-5"
      >
      ...
      <button
        @click="nextChapter"
        class="bg-slate-600 py-[10px] px-[20px] rounded-md ml-5"
      >
        <h1 class="inline text-[20px] text-white font-bold w-7 sm:hidden">
          &gt;
        </h1>
    ...
    <div
      v-for="image in chapterInfo"
      :key="image"
      class="w-full h-full m-2 relative bg-white rounded-[10px] flex justify-center items-center -z-10"
    >
      <img
        class="max-w-[300px] sm:max-w-[600px] smd:max-w-[800px] z-10"
        :src="image.img"
        alt="First slide"
      />
    </div>
  ...
  setup() {
    ...
    const previousChapter = () => {
      ...
      if (currentChapterMatch) {
        ...
        const newChapterUrl = chapterUrl.value.replace(
          /chapter-\d+/,
          `chapter-${nextChapterNumber}`
        ); // Replace the current chapter number with the incremented one
        ...
        console.error(
          "Could not parse chapter number from URL:",
          chapterUrl.value
        );
